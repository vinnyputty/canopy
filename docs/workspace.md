...
The open-issue picker selects its first result automatically. Up/Down moves the highlight without leaving the input; Enter opens the highlighted result, with issue keys and Jira URLs taking precedence. Search starts after two characters and a short pause. Exact and prefix summary matches lead the loaded results; project context comes from the active tab on that connection, otherwise its latest recent root. Loading, empty results, and recoverable errors have explicit states. Load more fetches another page and may reorder results by relevance, while preserving the selected issue; Retry retains already loaded matches.
