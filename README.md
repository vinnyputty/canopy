...
- Press `Space` on a focused issue row to preview its metadata, description, recent comments, and directional links. Open Development in the preview to load available pull request, commit, and remote link references. Resize the preview with its drag handle or arrow keys; `Escape` closes it and restores tree focus.
