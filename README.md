...
- Press `Enter` to edit the focused field (or the selected row’s summary), `Escape` to cancel, and `Tab` / `Shift+Tab` to advance between editable cells. Use `↑` / `↓` to choose assignees and status transitions. Summary drafts survive background refreshes.
